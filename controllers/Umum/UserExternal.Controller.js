// userExternalController.js
require('dotenv').config();
// const createUserExternalModel = require("../models/userExternalModels");
const bcrypt = require("bcrypt");
const jwt = require("jsonwebtoken");

// let UserExternal
// async function getUserExternal() {
//   UserExternal = await createUserExternalModel();
// }
...
async function createUserOne(req, res) {
  try {
    ...
    const user = await UserExternal.findOne({ email });
    ...
    const newUser = await UserExternal.create(req.body);
...
async function updateUserOne(req, res) {
  try {
    ...
    const dataUser = await UserExternal.findById(userId);
    if (!dataUser) {
      return res.status(404).json({ message: "User not found" });
    }
    ...
    const updatedUser = await UserExternal.findByIdAndUpdate(userId, newData, { new: true });
...
async function createUser(req, res) {
  try {
    ...
    const newCompany = await UserExternal.create(data);
